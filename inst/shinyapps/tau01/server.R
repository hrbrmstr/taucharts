...
shinyServer(function(input, output) {
    ...
    structure(
      list(
        team = c("d", "d", "d", "d", "l", "l", "l", "l",
                 "k", "k", "k", "k"),
        cycleTime = c(1L, 2L, 3L, 4L, 2L, 3L, 4L,
                      5L, 2L, 3L, 4L, 5L),
        effort = c(1L, 2L, 3L, 4L, 1L, 2L, 3L, 4L,
                   4L, 5L, 6L, 8L),
        count = c(1L, 5L, 8L, 3L, 1L, 5L, 8L, 3L, 1L,
                  5L, 8L, 3L),
        priority = c(
      ...
      .Names = c("team",
                 "cycleTime", "effort", "count", "priority"), class = "data.frame",
      row.names = c(NA, 12L)
    )
      ...
      list(
        ...
        priority = c(
          ...
          "low", "medium", "high", "low", "low", "medium", "high"
                    ...
                    "cycleTime", "effort", "count", "priority"),
  ...
  splom_dat <-
    structure(
      ...
      ), .Names = c("param1", "param2", "value1", "value2"), class = "data.frame", row.names = c(NA, 81L)
    )

  output$bar <- renderTaucharts({
    tauchart(bar_dat[runif(nrow(bar_dat), max = 3) <= input$data,]) %>%
      tau_bar("team", "effort", color = "priority") %>%
      tau_legend() %>% tau_tooltip()
  })

  output$line <- renderTaucharts({
    tauchart(line_dat[runif(nrow(line_dat), max = 3) <= input$data,]) %>%
